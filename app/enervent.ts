...
export interface HoldingRegisterSettingConfiguration extends BaseSettingConfiguration {
    ...
    min?: number
    max?: number
    // When set (>= 2), write this many consecutive holding registers as one
    // "write multiple registers" (FC16) block instead of a single register (FC6).
    // Some registers (e.g. the panel ventilation level, 53) are only honored by
    // the unit when written as a block — the native Home Assistant modbus setup
    // wrote reg 53 as a 2-register block. The block is read-modify-write: only
    // the first register (dataAddress) is set; the trailing registers keep their
    // current values, so nothing else is clobbered.
    writeMultiple?: number
}
...
export const AVAILABLE_SETTINGS: Record<string, SettingConfiguration> = {
    ...
    'ventilationLevel': {
        ...
        min: 20,
        max: 100,
        // Reg 53 is not honored as a single-register (FC6) write, nor as a
        // 1-register FC16 write (the fans blip then revert). The unit only holds
        // it when written as a 2-register FC16 block (regs 53+54), like the old
        // native modbus. Read-modify-write preserves reg 54 (supplyFanOverPressure)
        // instead of zeroing it as the old native `[value, 0]` did.
        writeMultiple: 2,
    },
